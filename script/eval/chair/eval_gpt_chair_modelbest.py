...
from gpt4_grpc import ChatClient
    ...
    for cap in caps:
        ...
        if "The following is a response without hallucination." in cap_text:
            new_cap_text = cap_text.split(
                "The following is a response without hallucination.")[-1].strip()
        elif "The following is a response with hallucination." in cap_text:
            new_cap_text = cap_text.split(
                "The following is a response with hallucination.")[0].strip()
        elif "Generate a response without errors." in cap_text:
            new_cap_text = cap_text.split(
                "Generate a response without errors.")[-1].strip()
        elif "Generate a response with errors." in cap_text:
            new_cap_text = cap_text.split(
                "Generate a response with errors.")[0].strip()
        else:
...
def get_inverse_synonym_dict(path):
    if path.endswith('.txt'):
        synonyms = open('script/eval/data/synonyms_refine.txt').readlines()
        synonyms = [s.strip().split(', ') for s in synonyms]
        mscoco_objects = []  # mscoco objects and *all* synonyms
        inverse_synonym_dict = {}
        for synonym in synonyms:
            new_synonym = [s.strip() for s in synonym]
            mscoco_objects.extend(new_synonym)
            for s in new_synonym:
                inverse_synonym_dict[s] = new_synonym[0]
        ...
        mscoco_objects = []
        inverse_synonym_dict = {}
        for cls in synonyms.keys():
            new_synonym = list(set([s.strip() for s in synonyms[cls]] + [cls]))
            mscoco_objects.extend(new_synonym)
            for s in new_synonym:
                inverse_synonym_dict[s] = cls

    return mscoco_objects, inverse_synonym_dict

class CHAIR(object):

    def __init__(self, imids, coco_path, synonym_path='data/synonyms_refine.txt'):

        self.imid_to_objects = {imid: [] for imid in imids}

        self.coco_path = coco_path

        self.chat_model = ChatClient()
        self.model_id = 32
        self.fail_limit = 50

        # read in synonyms
        # synonyms = open('data/synonyms_refine.txt').readlines()
        # synonyms = [s.strip().split(', ') for s in synonyms]
        ...
        self.mscoco_objects, self.inverse_synonym_dict = get_inverse_synonym_dict(synonym_path)

        coco_double_words = [word for word in self.inverse_synonym_dict.keys() if len(
            word.strip().split(' ')) >= 2]
    ...
    def get_annotations_from_captions(self):
        ...
        for i, annotation in enumerate(caption_annotations):
            ...
            imid = annotation['image_id']
            if imid in self.imid_to_objects:
                _, node_words, _, _ = self.caption_to_words(
                    annotation['caption'])
                self.imid_to_objects[imid].update(
                    [item[-1] for item in node_words])
        print("\n")
    ...
    def get_gpt_resp(self, data_item):
        ...
        while True:
            if len(data_item["caption"].strip().split()) <= 3:
                ...
                print(
                    f"**[Short Answer]**@{data_item['caption']}@", data_item["extract_objs"])
            ...
            resp = None
            try:
                result, resp = self.chat_model.chat_sync_retry(
                    user_prompt=prompt, model_id=self.model_id, max_tokens=512)

                # Logging consumption
                used_tokens["total"] += resp['data']['usage']["totalTokens"]
                used_tokens["input"] += resp['data']['usage']["promptTokens"]
                used_tokens["output"] += resp['data']['usage']["completionTokens"]
                ...
                success_tokens = {"total": resp['data']['usage']["totalTokens"],
                                  "input": resp['data']['usage']["promptTokens"],
                                  "output": resp['data']['usage']["completionTokens"]}
                ...
                time.sleep(random.random()*10 + fail_cnt)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                ...
                try:
                    ...
                    all_used_tokens = {
                        key: all_used_tokens[key] + used_tokens[key] for key in all_used_tokens.keys()}
                    all_success_tokens = {
    ...
    def compute_chair(self, cap_file, sample_num, gpt_process=False, org_dir=None):
        ...
        if gpt_process:
            try:
                caps, all_used_tokens, all_success_tokens = load_prev_chair_results(cap_file)
        ...
        for i, cap_eval in enumerate(caps):
            ...
            if gpt_process:
                ...
                words, node_words, idxs = self.caption_objects_to_coco_objects(
                    ext_objs)
                raw_words = ext_objs
            else:
...
def load_imids(org_dir):
    if org_dir.endswith(".jsonl"):
        org_data_path = org_dir
    else:
        raise ValueError("org_dir should be a .jsonl file path!")
    org_data = read_jsonl(org_data_path)
    imgids = [int(org_data[i]["image_id"]) for i in range(len(org_data))]
...
def load_generated_captions(cap_file, org_dir=None):
    if cap_file.endswith(".json"):
        ...
        for i in range(len(caps)):
            ...
            elif "raw_question" in caps[i].keys():
                question = caps[i]["raw_question"]
            else:
                raise Exception(
                    "Expect 'question' or 'prompt' in generated file")
            ...
            new_item = {"image_id": imgid,
                        "question": question, "caption": answer}
            new_caps.append(new_item)
        caps = new_caps
        imids = set(imids)
...
def load_prev_chair_results(cap_file):
    tag = cap_file.split('/')[-1].replace(".jsonl", "")
    save_path = os.path.join(os.path.dirname(cap_file), f'hall_{tag}_-1.json')
    ...
    for item in results:
        ...
        new_item['objs'] = item['words']
        caps.append(new_item)

    all_used_tokens = data['overall_metrics']['all_gpt_used_tokens']
    all_success_tokens = data['overall_metrics']['all_gpt_success_tokens']

    return caps, all_used_tokens, all_success_tokens

def save_hallucinated_words(cap_file, cap_dict, save_file, sample_num):
    tag = cap_file.split('/')[-1].replace(".jsonl", "")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--org_folder", type=str, default='script/eval/data/chair_instruction2_img_diversify_8_noemo_test_300.jsonl')
    parser.add_argument("--cap_type", type=str, default='')
    parser.add_argument("--coco_path", type=str,
                        default='script/eval/data/annotations')
    parser.add_argument("--sample_num", type=int, default=-1)
    parser.add_argument("--use_gpt", action='store_true')
    parser.add_argument("--do_rescore", action='store_true')
    args = parser.parse_args()

    print("use gpt:", args.use_gpt)
    if args.cap_folder != '':
        patterns = ['*', '*/*', '*/*/*', '*/*/*/*']
        f_list = sum([list(glob.glob(args.cap_folder + p))
                     for p in patterns], [])
        print(f_list)
        cap_file_list_path = [x for x in f_list if x.endswith(
            '.jsonl') and args.cap_type in x]
    ...
    imids = load_imids(args.org_folder)
    # _, imids, _ = load_generated_captions(
    #     args.cap_file, org_dir=args.org_folder)
    # assert len(imids) == 300

    print("=======init evaluator=======")
    if args.do_rescore:
        synonym_path = 'script/eval/data/chair_mapping.mergeall.correct_0406.json'
    else:
        synonym_path = 'script/eval/data/synonyms_refine.txt'
    evaluator = CHAIR(imids, args.coco_path, synonym_path=synonym_path)
    ...
    for path in cap_file_list_path:
        ...
        if not args.do_rescore:
            target_save_path_new = save_dir / f'hall_{tag}_{args.sample_num}.json'
        else:
            target_save_path_new = save_dir / f'hall_{tag}_{args.sample_num}_mapping0406.json'
        ...
        save_hallucinated_words(path, cap_dict, target_save_path_new,
