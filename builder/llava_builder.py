...
class LLaVABuilder(ModelBuilder):
    ...
    def build(cls, model_path, model_base, model_name, **kwargs):
        ...
        if "customized_config" in kwargs:
            customized_config = kwargs["customized_config"]
        else:
        ...
        if "overwrite_config" in kwargs:
            overwrite_config = kwargs["overwrite_config"]
        else:
        ...
        if "llava" in model_name.lower() or "rlaif" in model_name.lower() or is_multimodal:
            # Load LLaVA model
            ...
            if "lora" in model_name.lower() and model_base is not None:
                ...
                if "mixtral" in model_name.lower():
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    model = LlavaMixtralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True,
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    model = LlavaGemmaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True,
                                                                  config=lora_cfg_pretrained,
                                                                  attn_implementation=attn_implementation, **kwargs)
                else:
                ...
                if "mixtral" in model_name.lower():
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                    llava_cfg = LlavaConfig.from_pretrained(model_path)
                    model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=llava_cfg,
                                                                  **kwargs)
                else:
                ...
                if "mixtral" in model_name.lower():
                    ...
                    tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)
                    cfg_pretrained = AutoConfig.from_pretrained(model_path)
                    model = LlavaGemmaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True,
                                                                  config=cfg_pretrained,
                                                                  attn_implementation=attn_implementation, **kwargs)
                else:
                    try:
